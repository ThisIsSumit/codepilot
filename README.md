...
```
GitHub Webhook → Go (Gin) → Redis Queue → Worker → Claude Agent (Tool Use)
                                                          ↓
...
| AI Agent | Claude claude-opus-4-6 (tool use: read_pr_diff, report_issues, post_github_comment) |
...
Set env vars in Railway dashboard: `DATABASE_URL`, `REDIS_URL`, `ANTHROPIC_API_KEY`, `GITHUB_TOKEN`, `GITHUB_WEBHOOK_SECRET`
...
- **Built an autonomous AI code reviewer** using Claude tool-use (claude-opus-4-6) that reads PR diffs, detects bugs/security issues, and posts structured GitHub reviews with zero human intervention
...
│   ├── internal/
│   │   ├── agent/agent.go          ← Claude tool-use loop
